...
from minigauss.priors import PeriodicKernel, ConstantFunc, Bound
from minigauss.priors.mean import PolynomialFunc


def test_function_1D(x):
    # return 0.25 * x + np.sin(x) + np.sin(1/4 * x) - 2*np.sin(3/5 * x) # TODO: Composable priors!
    return np.sin(3 / 5 * x * np.pi)
